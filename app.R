...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # Matching response provided in helpers::HHLvals
      radioButtons("hhl_corr","Household Language", choiceNames=c("All","English only","Spanish", "Other"),
                                                   choiceValues=c(0, 1, 2, 5)),
      # Matching response provided in helpers::FSvals
      radioButtons("fs_corr","SNAP Recipient", choiceNames=c("All", "Yes", "No"),
                                               choiceValues=c(0,1,2)),
      # Need some clarity on completing this one, it does not match neatly with another variable item.
